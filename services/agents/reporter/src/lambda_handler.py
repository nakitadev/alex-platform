    ...
    with trace("Reporter Agent"):
        ...
        try:
            ...
            if openrouter_api_key and not (os.getenv("USE_OPENROUTER", "false").lower() in ("true", "1")):
                logger.warning(f"Reporter: Primary model failed ({e}), falling back to OpenRouter...")
                fb_model, fb_tools, fb_task, fb_context = create_agent(job_id, portfolio_data, user_data, db, force_openrouter=True)
                fb_agent = Agent[ReporterContext](
                    name="Report Writer", instructions=REPORTER_INSTRUCTIONS, model=fb_model, tools=fb_tools
                )
                result = await Runner.run(
                    fb_agent,
                    input=fb_task,
