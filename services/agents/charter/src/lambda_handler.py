    ...
    with trace("Charter Agent"):
        ...
        try:
            ...
            if openrouter_api_key and not (os.getenv("USE_OPENROUTER", "false").lower() in ("true", "1")):
                logger.warning(f"Charter: Primary model failed ({e}), falling back to OpenRouter...")
                fb_model, fb_task = create_agent(job_id, portfolio_data, db, force_openrouter=True)
                fb_agent = Agent(
                    name="Chart Maker",
                    instructions=CHARTER_INSTRUCTIONS,
                    model=fb_model
