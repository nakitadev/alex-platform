        ...
        with trace("Planner Orchestrator"):
            ...
            try:
                ...
                if openrouter_api_key and not (os.getenv("USE_OPENROUTER", "false").lower() in ("true", "1")):
                    logger.warning(f"Planner: Primary model failed ({e}), falling back to OpenRouter...")
                    fb_model, fb_tools, fb_task, fb_context = create_agent(job_id, portfolio_summary, db, force_openrouter=True)
                    fb_agent = Agent[PlannerContext](
                        name="Financial Planner",
                        instructions=ORCHESTRATOR_INSTRUCTIONS,
                        model=fb_model,
                        tools=fb_tools
