    ...
    with trace("Retirement Agent"):
        ...
        try:
            ...
            if openrouter_api_key and not (os.getenv("USE_OPENROUTER", "false").lower() in ("true", "1")):
                logger.warning(f"Retirement: Primary model failed ({e}), falling back to OpenRouter...")
                fb_model, fb_tools, fb_task = create_agent(job_id, portfolio_data, user_preferences, db, force_openrouter=True)
                fb_agent = Agent(
                    name="Retirement Specialist",
                    instructions=RETIREMENT_INSTRUCTIONS,
                    model=fb_model,
