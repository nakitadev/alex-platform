...
def get_model(force_openrouter: bool = False) -> LitellmModel:
    """Get LitellmModel with OpenRouter fallback support."""
    use_openrouter = force_openrouter or os.getenv("USE_OPENROUTER", "false").lower() in ("true", "1")
    openrouter_api_key = os.getenv("OPENROUTER_API_KEY")
    if use_openrouter and openrouter_api_key:
        openrouter_model = os.getenv("OPENROUTER_MODEL", "openrouter/openai/gpt-4o-mini")
        os.environ["OPENROUTER_API_KEY"] = openrouter_api_key
        logger.info(f"Tagger: Using OpenRouter model {openrouter_model}")
        return LitellmModel(model=openrouter_model)
...
async def classify_instrument(
    ...
    try:
        ...
        openrouter_api_key = os.getenv("OPENROUTER_API_KEY")
        if openrouter_api_key and not (os.getenv("USE_OPENROUTER", "false").lower() in ("true", "1")):
            logger.warning(f"Tagger: Primary model failed ({e}), falling back to OpenRouter...")
            try:
                fallback_model = get_model(force_openrouter=True)
                with trace(f"Classify {symbol} (OpenRouter Fallback)"):
                    agent = Agent(
                        name="InstrumentTagger",
                        instructions=TAGGER_INSTRUCTIONS,
                ...
                logger.error(f"Tagger: OpenRouter fallback also failed: {fb_err}")
                raise fb_err from e
